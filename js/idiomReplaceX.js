 ...
 *
 * @param node
 * @returns {{node, innerText: *, children: *[], htmlChecksum: *}|null}
 *
 *   {
 *     htmlChecksum:
 *     textLength:
 ...
 *   }
 */
var textContainerData = function(node) {
  if(node) {
    if (isTextBlock(node)) {
...
var collectSubTreeInnerTexts = function(subTextBlocks){
  const innerTexts = [];
 ...
 * @returns {*[]}
 */
var findTextBlockElements = function (siblingNodes) {

  var siblingTextBlocks = [];
  siblingNodes.forEach(function(siblingNode) {
    // 1. descend into children
    var childrenTexBlocks = findTextBlockElements(siblingNode.childNodes);
 ...
 * @returns {*}
 */
var flattenTextBlockTree = function(rootTextBlocks, flatList){
...
var textBlockElements = flattenTextBlockTree(findTextBlockElements(document.body.childNodes), []);
